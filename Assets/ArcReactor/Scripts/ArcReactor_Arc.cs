...
public class ArcReactor_Arc : MonoBehaviour {
	...
	{
		oldShapeTransformsSize = shapeTransforms.Length;
		if (closedShape)
		{
			shapeKeyLocations = new float[shapeTransforms.Length+1];
			shapeKeyNormalizedLocations = new float[shapeTransforms.Length+1];
		}
		else
		{
			shapeKeyLocations = new float[shapeTransforms.Length];
			shapeKeyNormalizedLocations = new float[shapeTransforms.Length];
		}
		shapeTangents = new Vector3[shapeTransforms.Length];
	}
	...
	void Start () 
	{
		...
		if (shapeTransforms.Length<2)
		{
			Debug.LogError(gameObject.name + " : There should be at least 2 shape transforms for correct shape calculation. Deactivating component");
			this.enabled = false;
			return;
		}
		if (arcs.Length == 0)
		...
		for (int i = 0; i < arcs.Length; i++)
		{
			...
			{
				Debug.LogWarning(gameObject.name + " : Segment length of Arc #"+i.ToString()+" is set to zero, arc always will be consisting of only 2 vertexes");
			}
